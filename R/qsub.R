...
#' @export
qsub <- function(path, args = NA, watch = FALSE, ...){
  ...
  if (get_jobwatcher_mode() %in% c("hgc", "uge")) {
    ...
    if (!is.na(args)) rlang::warn("This is not UGE environment. argument 'args' will be ignored.")
    if (fs::file_access(path, 'execute') && !is.na(read_shebang())) command <- path
    else {
      rlang::inform("Your file is not executable or shebang is not detected in your file. Your file will be executed on '/bin/sh'.")
      command <- paste0("/bin/sh ", path)
      }
    system(command, intern = FALSE, wait = watch)
